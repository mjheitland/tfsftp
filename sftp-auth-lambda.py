...
def lambda_handler(event, context):
    ...
    resp = get_secret_from_secretsmanager(PARAMETER_PREFIX + input_username)
    # resp = get_secret_from_parameterstore(PARAMETER_PREFIX + input_username)
    
    if resp != None:
        resp_dict = json.loads(resp)
    else:
        print("Exception thrown, cannot read secret value for user {}{} and ServerId: {}"
          .format(PARAMETER_PREFIX, input_username, event['serverId']));
        return {}
...
def get_secret_from_secretsmanager(id):
    region = os.environ['AWS_REGION']
    print("SSM Region: " + region)
...
def get_secret_from_parameterstore(id):
    region = os.environ['AWS_REGION']
    print("SSM Region: " + region)
    ...
    try:
        ...
        print('Error talking to Parameter Store: ' + err.response['Error']['Code'] + ', Message: ' + str(err))
        return None
